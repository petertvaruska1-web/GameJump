...
export class Bot {
  ...
  private pick(f: Fight) {
    ...
    for (const q of f.fighters) {
      ...
      const dy = q.p.pos.y - this.pos.y;
      let d = Math.hypot(q.p.pos.x - this.pos.x, q.p.pos.z - this.pos.z) + Math.abs(dy) * 1.5;
  ...
  private skitter(dt: number, f: Fight) {
    ...
    if (!stunned) {
      ...
      switch (this.state) {
        ...
        case BotState.Hunt: {
          if (!T) break;
          const dx = T.p.pos.x - this.pos.x, dz = T.p.pos.z - this.pos.z, d = Math.hypot(dx, dz) || 1;
          const dy = T.p.pos.y - this.pos.y;
        ...
        case BotState.Tell:
          ...
          if (T) this.yaw = Math.atan2(T.p.pos.x - this.pos.x, T.p.pos.z - this.pos.z);
        ...
        case BotState.Strike:
          ...
          if (T && f.targetable(T)) {
            const dx = T.p.pos.x - this.pos.x, dz = T.p.pos.z - this.pos.z, dy = T.p.pos.y + 0.6 - this.pos.y;
  ...
  private pounce(f: Fight, T: Fighter): boolean {
    ...
    const dx = T.p.pos.x - this.pos.x, dz = T.p.pos.z - this.pos.z, d = Math.hypot(dx, dz) || 1;
    const lx = T.p.pos.x - (dx / d) * 1.4, lz = T.p.pos.z - (dz / d) * 1.4;
    f.world.groundProbe(lx, lz, 0.3, T.p.pos.y + 0.6, gh);
    if (!gh.c || Math.abs(gh.top - T.p.pos.y) > 1.2) return false;
  ...
  private wasp(dt: number, f: Fight) {
    ...
    if (!stunned) {
      ...
      if (T) {
        ...
        this.orbit += dt * 0.35;
        gx = T.p.pos.x + Math.sin(this.orbit) * this.keep;
        gz = T.p.pos.z + Math.cos(this.orbit) * this.keep;
        gy = Math.max(T.p.pos.y, ARENA.y) + this.hover;
        const dx = T.p.pos.x - p.x, dz = T.p.pos.z - p.z;
        this.yaw = Math.atan2(dx, dz);
    ...
    // steer (a knocked wasp drifts until it rights itself)
  ...
  update(dt: number, f: Fight) {
    ...
    for (let i = 0; i < sub && !this.dead; i++) {
      ...
      if (this.turnedBy) {
        ...
        const q = f.fighterAt(this.pos, R + 0.3);
        if (q) {
