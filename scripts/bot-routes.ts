...
// timing laser gates, jumping low beams, sliding under low gaps and high beams,
...
// controller forward) to prove the level can be completed. Enemies are not simulated.
...
import { GRAPPLE, PHYS, PLAYER, SLIDE, ZIP } from '../shared/constants';
...
const routes: Record<string, { start: V3; steps: Step[] }> = {
  center: {
    start: S,
...
function run(name: string, start: V3, pts: P[]): boolean {
  const world = new CollisionWorld(level);
  const m = new PlayerMotor();
  m.spawn(start[0], start[1], start[2], 0);
  const b = m.body;
  let i = 0, t = Number(process.env.T0 ?? 0), best = Infinity, bestT = t, jumpHold = 0, waited = 0;
  const log: string[] = [];
  const stats = { zips: 0, pads: 0, laserWaits: 0, laserJumps: 0, slides: 0, swings: 0 };
  let airT = 0;
  ...
  while (t < 600 && i < pts.length) {
    ...
    if (t - bestT > 20 && !m.zip) { log.push(`stuck near ${fmt(b.pos)} heading to #${i} ${fmt3(tgt.p)}`); break; }
    ...
    if (m.zip) { inp.x = 0; inp.z = 0; inp.sprint = false; }
      ...
      const ahead = predictLaser(b.pos.x, b.pos.y, b.pos.z, dirX * sp + beltX, dirZ * sp + beltZ, t, 0.7, b.height);
      const here = predictLaser(b.pos.x, b.pos.y, b.pos.z, beltX, beltZ, t, 0.5, b.height);
      ...
      if (here?.high && !m.sliding && !inp.slidePressed && here.dt < 0.4) {
        ...
        if (hs >= SLIDE.MIN_START) { inp.slidePressed = true; }
          ...
          const fw = predictLaser(b.pos.x, b.pos.y, b.pos.z, dirX * PLAYER.SPRINT_SPEED, dirZ * PLAYER.SPRINT_SPEED, t, 0.6, b.height);
          const bw = predictLaser(b.pos.x, b.pos.y, b.pos.z, -dirX * PLAYER.SPRINT_SPEED, -dirZ * PLAYER.SPRINT_SPEED, t, 0.6, b.height);
      ...
      if (tgt.kind === 'zip' && dist < 0.7) {
        ...
        if (zipSafe(zl, t, 0.02, b.pos.x, b.pos.z)) jump = true; else wait = true;
      }
    ...
    if (laserHit(level, b.pos.x, b.pos.y, b.pos.z, t, PLAYER.RADIUS, b.height - 0.1)) { log.push(`LASER near ${fmt(b.pos)} heading to #${i} ${fmt3(tgt.p)} (t=${t.toFixed(1)})`); break; }
    if (!m.zip && !m.grapple && (b.pos.y < m.lastGroundY - 10 || b.pos.y < level.killY)) { log.push(`FELL near ${fmt(b.pos)} heading to #${i} ${fmt3(tgt.p)} (t=${t.toFixed(1)})`); break; }
  }
...
for (const [name, r] of Object.entries(routes)) {
  ...
  if (!run(name, r.start, expand(r.steps))) fails++;
}
